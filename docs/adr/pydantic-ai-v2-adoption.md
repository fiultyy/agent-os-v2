...
| **docs curl :8000 归档** | ✅ **已归档(banner)** | 对抗验证订正:此前"零命中"为假(勘察时 cwd 路径误判 `ls docs/*.md` 在 services/orchestrator 下跑)。实际 `docs/` **11 文件 37 处 `:8000` 死引用**(curl 指向 commit c594969 删除的 gateway BFF)。10 文件(9 .md + architecture-report.html)加 DEPRECATED banner 标注死引用 + 现役端口(orchestrator:8001/observe:8002/native /h);历史快照不逐 curl 改(gateway 路由≠orchestrator 路由,逐改+验证=过度,ponytail) |
