    ...
    if _SKILL_TOOLS_AVAILABLE:
        ...
        _PRIMITIVE_TOOLS: list[tuple] = [
            ("http_get", http_get, "HTTP GET 请求",
             {"type": "object", "properties": {"url": {"type": "string"}, "headers": {"type": "object"}, "timeout": {"type": "integer"}}, "required": ["url"]}),
            ("http_post", http_post, "HTTP POST 请求",
             {"type": "object", "properties": {"url": {"type": "string"}, "body": {"type": "string"}, "headers": {"type": "object"}, "timeout": {"type": "integer"}}, "required": ["url"]}),
            ...
            ("file_read", file_read, "读取文件内容",
             {"type": "object", "properties": {"path": {"type": "string"}, "encoding": {"type": "string"}, "limit": {"type": "integer"}}, "required": ["path"]}),
            ("file_write", file_write, "写入文件内容",
             {"type": "object", "properties": {"path": {"type": "string"}, "content": {"type": "string"}, "encoding": {"type": "string"}, "mode": {"type": "string"}}, "required": ["path", "content"]}),
            ...
            ("file_list", file_list, "列出目录内容",
             {"type": "object", "properties": {"path": {"type": "string"}, "pattern": {"type": "string"}, "recursive": {"type": "boolean"}, "max_depth": {"type": "integer"}}, "required": ["path"]}),
            ("file_mkdir", file_mkdir, "创建目录",
             {"type": "object", "properties": {"path": {"type": "string"}, "parents": {"type": "boolean"}, "exist_ok": {"type": "boolean"}}, "required": ["path"]}),
            ("db_query", db_query, "执行 SELECT 查询",
             {"type": "object", "properties": {"sql": {"type": "string"}, "params": {"type": "array"}, "db_path": {"type": "string"}, "fetch": {"type": "string"}, "limit": {"type": "integer"}}, "required": ["sql"]}),
            ("db_execute", db_execute, "执行 INSERT/UPDATE/DELETE",
             {"type": "object", "properties": {"sql": {"type": "string"}, "params": {"type": "array"}, "db_path": {"type": "string"}, "commit": {"type": "boolean"}}, "required": ["sql"]}),
            ("db_transaction", db_transaction, "事务执行多条 SQL",
             {"type": "object", "properties": {"statements": {"type": "array"}, "db_path": {"type": "string"}}, "required": ["statements"]}),
            ("db_schema", db_schema, "获取表结构",
             {"type": "object", "properties": {"table": {"type": "string"}, "db_path": {"type": "string"}}, "required": ["table"]}),
        ]
        _SKILL_TOOLS: list[tuple] = [
            ("code_read", code_read, "读取代码文件",
             {"type": "object", "properties": {"path": {"type": "string"}, "language": {"type": "string"}, "start_line": {"type": "integer"}, "end_line": {"type": "integer"}, "highlight": {"type": "boolean"}}, "required": ["path"]}),
            ("code_write", code_write, "写入代码文件",
             {"type": "object", "properties": {"path": {"type": "string"}, "content": {"type": "string"}, "language": {"type": "string"}, "backup": {"type": "boolean"}, "atomic": {"type": "boolean"}, "encoding": {"type": "string"}}, "required": ["path", "content"]}),
            ("code_search", code_search, "搜索代码文件",
             {"type": "object", "properties": {"query": {"type": "string"}, "path": {"type": "string"}, "pattern_type": {"type": "string"}, "file_filter": {"type": "string"}, "case_sensitive": {"type": "boolean"}, "context_lines": {"type": "integer"}, "max_results": {"type": "integer"}, "recursive": {"type": "boolean"}, "max_depth": {"type": "integer"}}, "required": ["query"]}),
        ]
        ...
        def _bulk_register(registry, items):
            n = 0
            for name, handler, desc, params in items:
                registry.register(
                    name, handler, description=desc, parameters=params,
                )
                n += 1
            return n
        ...
        try:
            ...
            _tool_registry.register(
                ...
                parameters=CREATE_AGENT_SCHEMA,
            )
            _n_creator = 1
        ...
        logger.info(
            "tool register: primitive=%d skill=%d workflow=%d a2a=%d creator=%d total=%d | list_tools=%d "
