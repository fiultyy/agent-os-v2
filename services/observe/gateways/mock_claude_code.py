#!/usr/bin/env python3
"""Mock Claude Code Gateway: simulate claude-code turn sequence.

模拟 claude-code 行为，发送标准 turn 序列到 observe-service:
...
async def send_mock_turn(
    ...
    tool_call_event = {
        ...
        "data": {
            ...
            "tool_name": "Bash",
            "arguments": {"command": "echo 'hello from mock claude-code'", "timeout": 10000},
        },
    ...
    tool_result_event = {
        ...
        "data": {
            ...
            "result": "hello from mock claude-code\n",
            "error": "",
        },
    ...
    tick_completed_event = {
        ...
        "data": {
            "status": "success",
            "response": "Mock response from claude-code",
            "tool_count": 1,
            "duration_ms": 3000.0,
        },
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Mock Claude Code Gateway")
    parser.add_argument("--session-id", required=True, help="Unique session ID (uuid)")
    parser.add_argument(
        "--harness-id",
        default="mock-claude-code-1",
        help="Harness instance ID",
    )
    ...
    logger.info(f"Mock claude-code gateway connecting to {ws_url}")
