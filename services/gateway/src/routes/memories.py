...
async def list_memories(request: Request) -> list[dict[str, Any]] | dict[str, Any]:
    ...
    resp = await http_client.get(f"{ORCHESTRATOR_URL}/memories", params=params)
...
async def store_memory(request: Request) -> dict[str, Any]:
    ...
    resp = await http_client.post(f"{ORCHESTRATOR_URL}/memories", json=body)
    resp.raise_for_status()
    return resp.json()
...
async def get_memory_layers(request: Request) -> dict[str, Any]:
    ...
    params = dict(request.query_params)
    resp = await http_client.get(f"{ORCHESTRATOR_URL}/memories/layers", params=params)
    resp.raise_for_status()
    return resp.json()
...
async def delete_memory(memory_id: str) -> dict[str, Any]:
    """Delete a memory."""
    resp = await http_client.delete(f"{ORCHESTRATOR_URL}/memories/{memory_id}")
    resp.raise_for_status()
    return resp.json()
