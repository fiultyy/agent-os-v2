...
/**
 * Execution history panel — shows past execution runs and supports replay.
 *
 * Groups execution events by session/run, showing timestamps,
 * node counts, and total execution time.
 */
export function ExecutionHistoryPanel() {
  const events = useDebugStore((s) => s.executionEvents);
  const setReplayIndex = useDebugStore((s) => s.setReplayIndex);

  // Group events into runs (based on contiguous timestamps within 60s gaps)
          ...
          <div className="space-y-3">
            {runs.map((run, idx) => (
              <div key={idx} className="rounded-lg border bg-white p-3 shadow-sm">
                ...
                <button
                  onClick={() => {
                    // Find the first event of this run in the global list
...
interface RunSummary {
  startTime: string;
  totalTimeMs: number;
  events: typeof useDebugStore extends { getState: () => { executionEvents: infer T } } ? T : never;
  errors: number;
}
